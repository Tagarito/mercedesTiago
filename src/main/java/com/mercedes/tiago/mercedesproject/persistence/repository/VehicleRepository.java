...
//    @Query("select v from Vehicle v where v.model=?1")
    ...
    @Query("select v from Vehicle v where v.model = ?1 and v.fuel = ?2 and v.transmission =?3")
    List<Vehicle> getVehiclesWithAttributes(String model, String fuel, String transmission);
}
